import type { TranslationSchema } from "./en";

export const ar: TranslationSchema = {
  common: {
    back: "→ رجوع",
    loading: "جارٍ التحميل...",
    networkError: "تعذر الوصول إلى الخادم. تحقق من الاتصال وحاول مرة أخرى.",
    notAssigned: "غير محدد",
  },
  nav: {
    dashboard: "الرئيسية",
    myShipment: "شحناتي",
    addShipment: "إضافة شحنة",
    myAccount: "حسابي",
    settings: "الإعدادات",
    manifest: "قائمة اليوم",
    pickupScan: "مسح الاستلام",
    deliveryScan: "مسح التسليم",
    cod: "الدفع عند الاستلام",
  },
  login: {
    title: "تطبيق الشحن",
    subtitle: "سجّل الدخول للمتابعة.",
    apiKeyTab: "مفتاح API",
    credentialsTab: "اسم المستخدم وكلمة المرور",
    usernamePlaceholder: "اسم المستخدم أو البريد الإلكتروني",
    passwordPlaceholder: "كلمة المرور",
    companyDot: "دوت بوكس",
    companyKit: "تجريبي",
    company: "الشركة",
    defaultCompany: "الشركة الافتراضية",
    advancedOptions: "خيارات متقدمة",
    hideAdvancedOptions: "إخفاء الخيارات المتقدمة",
    sessionExpired: "انتهت صلاحية الجلسة. يرجى تسجيل الدخول مرة أخرى.",
    apiKeyPlaceholder: "مفتاح API",
    signingIn: "جارٍ تسجيل الدخول...",
    signIn: "تسجيل الدخول",
    connecting: "جارٍ الاتصال...",
    connect: "اتصال",
    loginFailed: "فشل تسجيل الدخول",
    incompleteAccount: "تم تسجيل الدخول لكن بيانات الحساب غير مكتملة (نوع المستخدم مفقود). تواصل مع الدعم الفني.",
  },
  dashboard: {
    title: "الرئيسية",
    welcome: "مرحبًا، {{name}}.",
    noShipments: "لا توجد شحنات قيد التنفيذ.",
    loading: "جارٍ تحميل الشحنات قيد التنفيذ...",
    pending: "الشحنات قيد التنفيذ",
    pendingCount: "قيد التنفيذ",
    totalCod: "إجمالي الدفع عند الاستلام",
  },
  myShipment: {
    title: "شحناتي",
    loading: "جارٍ تحميل الشحنات...",
    noShipments: "لا توجد شحنات مكتملة بعد.",
  },
  shipmentGrid: {
    previous: "السابق",
    next: "التالي",
    page: "صفحة {{page}}",
    orderId: "الشحنة",
    date: "التاريخ",
    status: "الحالة",
    recipient: "المستلم",
    description: "الوصف",
    cod: "الدفع عند الاستلام",
    selected: "المحدد: {{count}}",
    print: "طباعة",
    download: "تنزيل",
    printing: "جارٍ تجهيز ملف PDF...",
    printLimit: "يمكنك طباعة {{max}} شحنة كحد أقصى.",
  },
  orderPage: {
    title: "الشحنة {{orderId}}",
    loading: "جارٍ تحميل الشحنة...",
    error: "تعذر تحميل هذه الشحنة. تحقق من الاتصال وحاول مرة أخرى.",
    notFound: "الشحنة غير موجودة.",
    recipient: "المستلم",
    carrier: "شركة الشحن",
    charges: "الرسوم",
    codAmount: "مبلغ الدفع عند الاستلام",
    freightAmount: "رسوم الشحن",
  },
  addShipment: {
    title: "إضافة شحنة",
    loading: "جارٍ تحميل النموذج...",
    govPlaceholder: "اختر المحافظة",
    zonePlaceholder: "اختر المنطقة",
    zoneDisabledPlaceholder: "اختر المحافظة أولاً",
    streetPlaceholder: "عنوان الشارع",
    contactNamePlaceholder: "اسم المستلم",
    phonePlaceholder: "هاتف المستلم",
    phone2Placeholder: "هاتف المستلم (بديل، اختياري)",
    descriptionPlaceholder: "الوصف",
    codPlaceholder: "مبلغ الدفع عند الاستلام",
    estimatedFreight: "تقدير رسوم الشحن: {{amount}}",
    submit: "حفظ الشحنة",
    submitting: "جارٍ الحفظ...",
    saved: "تم حفظ الشحنة.",
    savedWithId: "تم حفظ الشحنة {{orderId}}.",
  },
  myAccount: {
    title: "حسابي",
    balance: "رصيد الحساب",
    balanceUnavailable: "رصيد الحساب غير متاح. سجّل الدخول مرة أخرى لتحديث حسابك.",
  },
  settings: {
    title: "الإعدادات",
    noSettings: "لا توجد إعدادات بعد.",
    notifications: "الإشعارات",
    language: "اللغة",
    signOut: "تسجيل الخروج",
  },
  notifications: {
    title: "الإشعارات",
    none: "لا توجد إشعارات بعد.",
    enabled: "الإشعارات مفعّلة على هذا الجهاز.",
    disabled: "فعّل الإشعارات لمعرفة تحديثات حالة شحنتك.",
    enable: "تفعيل الإشعارات",
    disable: "إيقاف الإشعارات",
    error: "حدث خطأ ما. تحقق من الاتصال وحاول مرة أخرى.",
    unsupported: "الإشعارات غير مدعومة على هذا المتصفح.",
    iosInstallRequired: "على آيفون/آيباد، أضف التطبيق إلى الشاشة الرئيسية أولاً، ثم عد إلى هنا لتفعيل الإشعارات.",
  },
  cod: {
    title: "تحصيل الدفع عند الاستلام",
    none: "لا توجد تحصيلات معلّقة.",
  },
  manifest: {
    title: "قائمة اليوم",
    agent: "المندوب: {{name}}.",
    noStops: "لا توجد محطات موكلة بعد.",
    loading: "جارٍ تحميل المحطات...",
    cod: "الدفع عند الاستلام",
  },
  scanPickup: {
    title: "مسح الاستلام",
    waybillLabel: "رقم بوليصة الشحن",
    waybillPlaceholder: "امسح أو أدخل رقم البوليصة",
    submit: "تسجيل الاستلام",
    queued: "تم إدراج الاستلام لـ {{id}}",
  },
  scanDelivery: {
    title: "مسح التسليم",
    waybillLabel: "رقم بوليصة الشحن",
    waybillPlaceholder: "امسح أو أدخل رقم البوليصة",
    submit: "تسجيل التسليم",
    queued: "تم إدراج التسليم لـ {{id}}",
    contact: "جهة الاتصال",
    actionsTitle: "إجراءات سير العمل",
    actionsLoading: "جارٍ تحميل الإجراءات…",
    actionsError: "تعذّر تحميل الإجراءات لهذا الطلب.",
    noActions: "لا توجد إجراءات متاحة لهذا الطلب.",
    actionRunning: "جارٍ التطبيق…",
    actionSuccess: "تم تطبيق الإجراء.",
    actionError: "تعذّر تطبيق الإجراء.",
    scanNext: "مسح التالي",
  },
  barcodeScanner: {
    scanButton: "المسح بالكاميرا",
    manualEntry: "إدخال يدوي",
    cancel: "إلغاء",
    loading: "جارٍ تشغيل الكاميرا…",
    permissionDenied: "تم رفض إذن الكاميرا. يرجى السماح بالوصول إلى الكاميرا أو إدخال الرقم يدويًا.",
    noCamera: "لم يتم العثور على كاميرا على هذا الجهاز. أدخل الرقم يدويًا.",
    genericError: "تعذّر تشغيل الكاميرا. أدخل الرقم يدويًا.",
  },
};
